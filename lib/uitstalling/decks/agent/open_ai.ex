defmodule Uitstalling.Decks.Agent.OpenAI do
  @moduledoc """
  Wire client for OpenAI-shaped providers (OpenRouter, most cheap-model
  aggregators): `POST <base>/chat/completions` with Bearer auth. All prompt/
  context assembly lives in `Agent.Context` — same prompts and validate-retry
  contract as the Anthropic-shaped client; only the wire format differs.
  Select with AGENT_API_FORMAT=openai.
  ...
  def generate_slide(deck, request, retry) do
    system = Context.edit_system_prompt() <> "\n" <> Context.edit_context_prompt(deck)

    with {:ok, api_key} <- Agent.fetch_api_key(),
         {:ok, text} <-
           call_api(api_key, system, Context.edit_user_prompt(deck, request, retry),
             max_tokens: 4096
           ) do
  ...
  def generate_ops(deck, request, retry) do
    system = Context.ops_system_prompt() <> "\n" <> Context.edit_context_prompt(deck)

    with {:ok, api_key} <- Agent.fetch_api_key(),
         {:ok, text} <-
           call_api(api_key, system, Context.ops_user_prompt(deck, request, retry),
             max_tokens: 2048
           ) do
      Context.extract_json(text)
    end
